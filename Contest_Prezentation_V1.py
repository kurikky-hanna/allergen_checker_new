...
if uploaded:
    ...
    # 💡 データベース内の最新の日付から「月」を自動判定する
    detected_month = 7  # デフォルト値
    if not df_dates.empty:
        first_date_str = df_dates["date"].iloc[0]  # 例: "7/1(水)"
    ...
    tab1, tab2 = st.tabs(["📅 カレンダー", "🍔 アレルゲンから探す"])
    ...
    with tab1:
        ...
        year = datetime.now().year
        month = detected_month  # 💡 PDFから解析した「月」を自動セット！
                ...
                if day == 0:
                    ...
                    with cols[idx]:
                        if not df_day_menu.empty:
                            ...
                            # 押せるボタン（給食がある日）
                            with st.popover(
                                f"**{day}**", use_container_width=True
                            ):
